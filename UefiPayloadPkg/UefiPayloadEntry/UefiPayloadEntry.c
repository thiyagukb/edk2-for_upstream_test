  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    NewSmbiosTable = BuildGuidHob (&gUniversalPayloadAcpiTableGuid, sizeof (SmBiosTableHob));
    ASSERT (NewSmbiosTable != NULL);
    CopyMem (NewSmbiosTable, &SmBiosTableHob, sizeof (SmBiosTableHob));
  ...
  if (!EFI_ERROR (Status)) {
    NewAcpiTableHob = BuildGuidHob (&gUniversalPayloadSmbiosTableGuid, sizeof (AcpiTableHob));
